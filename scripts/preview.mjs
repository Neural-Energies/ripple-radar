...
async function restart() {
  ...
  // Vite's Nitro bundle inlines PGLite but not pglite.data / wasm. Deployed
  // apps use Neon (DATABASE_URL) and never hit this path; local `vite preview`
  // does. Stage the assets next to the bundled module so bootstrap can open them.
  ...
  if (existsSync(pgliteDest) && existsSync(pgliteSrc)) {
    for (const name of ["pglite.data", "pglite.wasm", "initdb.wasm"]) {
      ...
      if (existsSync(src)) copyFileSync(src, join(pgliteDest, name));
    }
  }
